...
export async function generateMetadata({ params }: { params: Promise<{ topic: string }> }): Promise<Metadata> { const { topic } = await params; const guide = getGuide(topic); return guide ? { title: `${guide.title} Guide | RailSahayak`, description: guide.description, alternates: localizedAlternates(`/guides/${guide.slug}`) } : {}; }
