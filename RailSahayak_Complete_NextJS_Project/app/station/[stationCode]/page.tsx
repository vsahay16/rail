...
export async function generateMetadata({ params }: { params: Promise<{ stationCode: string }> }): Promise<Metadata> {
  ...
  return { title: `${code} Station: Arrivals & Departures | RailSahayak`, description: `See arrivals, departures and available platform information for station ${code}.`, alternates: localizedAlternates(`/station/${code}`) };
}
