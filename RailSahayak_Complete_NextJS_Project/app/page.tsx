...
const copy = {
  en: {
    ...
    journeyKicker: "Coming together as one flow", journeyTitle: "Your entire train journey on one calm screen.", journeyDescription: "Save a journey once and RailSahayak can bring together live status, chart timing, coach information and useful alerts.", journeyCta: "Start with your PNR",
  ...
  hi: {
    ...
    journeyKicker: "एक आसान यात्रा अनुभव", journeyTitle: "आपकी पूरी रेल यात्रा, एक शांत और सरल स्क्रीन पर।", journeyDescription: "यात्रा एक बार सेव करें और RailSahayak लाइव स्थिति, चार्ट समय, कोच जानकारी और जरूरी अलर्ट एक साथ दिखा सकता है।", journeyCta: "अपने PNR से शुरू करें",
...
function pickString(value: unknown, fallback = "—") { return typeof value === "string" || typeof value === "number" ? String(value) : fallback; }
...
function mapProviderResult(tab: SearchTab, payload: Record<string, unknown>, language: Language): SearchResult {
  ...
  if (tab === "pnr") return {
    tone: "success", eyebrow: hi ? "नवीनतम PNR उत्तर" : "Latest PNR response", title: pickString(source.trainName, pickString(source.status, hi ? "PNR स्थिति मिली" : "PNR status received")),
    description: hi ? "लाइव सेवा ने नवीनतम उपलब्ध रेलवे रिकॉर्ड दिया है। अंतिम यात्रा अधिकार IRCTC पर सत्यापित करें।" : "The live provider returned the latest available railway record. Always verify the final travel authority on IRCTC.",
    details: [{ label: hi ? "ट्रेन" : "Train", value: pickString(source.trainNumber) }, { label: hi ? "वर्तमान स्थिति" : "Current status", value: pickString(source.currentStatus, pickString(source.status)) }, { label: hi ? "चार्ट" : "Chart", value: pickString(source.chartStatus, pickString(source.chartPrepared)) }],
  };
  if (tab === "live") {
    const delay = source.delayMinutes === 0 ? (hi ? "समय पर" : "On time") : source.delayMinutes ? `${source.delayMinutes} ${hi ? "मिनट देरी" : "min late"}` : (hi ? "नवीनतम स्थिति मिली" : "Latest status received");
    return { tone: "success", eyebrow: hi ? "लाइव रनिंग अपडेट" : "Live running update", title: pickString(source.trainName, pickString(source.trainNumber, hi ? "ट्रेन मिल गई" : "Train located")), description: delay,
      details: [{ label: hi ? "वर्तमान स्टेशन" : "Current station", value: pickString(source.currentStation, pickString(source.currentHalt)) }, { label: hi ? "अगला स्टेशन" : "Next halt", value: pickString(source.nextStation, pickString(source.nextHalt)) }, { label: hi ? "अपडेट" : "Updated", value: pickString((payload.meta as Record<string, unknown> | undefined)?.timestamp, hi ? "अभी" : "Just now") }],
    };
  }
  return { tone: "success", eyebrow: hi ? "उपलब्ध ट्रेनें" : "Available trains", title: Array.isArray(source) ? `${source.length} ${hi ? "ट्रेनें मिलीं" : "trains found"}` : (hi ? "रूट विकल्प मिले" : "Route options received"), description: hi ? "मौजूदा समय-सारणी सेवा ने इन स्टेशनों के लिए उपलब्ध ट्रेन विकल्प दिए हैं।" : "The current timetable provider returned matching train options for this station pair.", details: [{ label: hi ? "डेटा स्थिति" : "Data status", value: hi ? "नवीनतम सेवा उत्तर" : "Latest provider response" }] };
}
...
export default function Home() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/rail?${params.toString()}`, { cache: "no-store" });
      ...
      if (!response.ok) {
        ...
        setResult({
          ...
          description: notConfigured ? (language === "hi" ? "सुरक्षित इंटीग्रेशन पहले से तैयार है। API कुंजी सर्वर पर रहती है और यात्रियों को दिखाई नहीं देती।" : "The secure integration is already built. The API key stays on the server and is never exposed to visitors.") : pickString(payload.message, language === "hi" ? "रेलवे डेटा सेवा ने उपयोगी उत्तर नहीं दिया।" : "The railway data provider did not return a usable result."),
        });
