...
export async function generateMetadata({ params }: { params: Promise<{ route: string }> }): Promise<Metadata> {
  ...
  return parsed ? { title: `${parsed.from} to ${parsed.to} Trains | RailSahayak`, description: `Find trains running between ${parsed.from} and ${parsed.to}.`, alternates: localizedAlternates(`/trains/${route.toLowerCase()}`) } : {};
}
