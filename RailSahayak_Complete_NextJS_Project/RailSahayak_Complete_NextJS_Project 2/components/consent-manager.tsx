...
export function ConsentManager({ gaId, adsenseClient }: { gaId?: string; adsenseClient?: string }) {
  ...
  return <>
    ...
    {consent === null && <aside className="consent-banner" role="dialog" aria-label="Privacy choices" aria-live="polite"><div><b>Privacy choices · गोपनीयता विकल्प</b><p>We use optional analytics to improve RailSahayak and advertising to fund it. Essential railway tools work without them. <Link href="/privacy">Read privacy policy</Link>.</p></div><div><button className="consent-secondary" onClick={() => choose("necessary")}>Necessary only</button><button className="consent-primary" onClick={() => choose("analytics_ads")}>Allow analytics & ads</button></div></aside>}
  </>;
}
